...
rest_subgraph_g = function(graph, df_base, df_restrict, cutoff = median(df_restrict$Value), simpl = F)
{
  l1 = intersect(df_restrict[[1]][df_restrict[[2]]>=cutoff], df_base$Bait)
  l2 = intersect(df_restrict[[1]][df_restrict[[2]]>=cutoff], df_base$Prey)
  li = unique(append(l1,l2))
  sg = induced_subgraph(graph,li)
  if (simpl == T)
  {
